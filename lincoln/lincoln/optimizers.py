...
class SGDMomentum(Optimizer):
    def __init__(
        self, lr: float = 0.01, final_lr: float = 0, decay_type: str = None, momentum: float = 0.9
    ) -> None:
        super().__init__(lr, final_lr, decay_type)
...
class AdaGrad(Optimizer):
    def __init__(
        self, lr: float = 0.01, final_lr_exp: float = 0, final_lr_linear: float = 0
    ) -> None:
    ...
    def step(self) -> None:
        ...
        for param, param_grad, sum_square in zip(
            self.net.params(), self.net.param_grads(), self.sum_squares
        ):
